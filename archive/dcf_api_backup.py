...
def _projection_inputs(last: pd.Series, df_num: pd.DataFrame,
                       params: Dict[str, float]) -> Dict[str, float]:
    inputs = {
        'revenue_start': float(_to_float(last.get("totalRevenue"), _last(df_num.get("totalRevenue"), 0.0)) or 0.0),
        'rev_start': float(np.nan_to_num(last.get("rev_growth"), nan=0.05)),
        'margin_start': float(np.nan_to_num(last.get("ebit_of_sales"), nan=0.15)),
        'dna_start': float(np.nan_to_num(last.get("dna_of_sales"), nan=0.03)),
        'capex_start': float(np.nan_to_num(last.get("capex_of_sales"), nan=0.05)),
        'nwc_start': float(np.nan_to_num(last.get("nwc_ratio"), nan=0.0)),
        'tax_start': float(np.nan_to_num(last.get("tax_of_ebit"), nan=DEFAULT_TAX_RATE)),
        'nwc_level': float(_to_float(last.get("netWorkingCapital"), 0.0) or 0.0)
    }
...
def calculate_dcf(ticker: str, parameters: Dict[str, float]) -> Dict[str, object]:
    ...
    inputs = _projection_inputs_yf(last_year, df_num, parameters)
